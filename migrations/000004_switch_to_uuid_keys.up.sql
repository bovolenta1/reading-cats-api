-- Drop existing reading tables (data is test/garbage only)
DROP TABLE IF EXISTS reading_goal CASCADE;
DROP TABLE IF EXISTS reading_day CASCADE;

-- Recreate reading_day with user_id as UUID FK to users(id)
CREATE TABLE IF NOT EXISTS reading_day (
  ...
  CONSTRAINT reading_day_user_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);

CREATE INDEX idx_reading_day_user_date ON reading_day(user_id, reading_date DESC);

-- Recreate reading_goal with user_id as UUID FK to users(id)
CREATE TABLE IF NOT EXISTS reading_goal (
  ...
  CONSTRAINT reading_goal_user_fk FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
);

CREATE INDEX idx_reading_goal_user_start_date ON reading_goal(user_id, start_date DESC);

-- Recreate updated_at trigger for reading_goal
